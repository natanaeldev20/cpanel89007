...
const CreateAuthority = () => {
  ...
  const onSubmit = async (data: CreateAuthorityData) => {
    setLoading(true);
    ...
    try {
      ...
      const res = await createAuthority(formData);

      if (res.status === 201 || res.status === 200) {
        toast.success("Autoridad creada con exito", {
    ...
    <>
      ...
      >
        <div className="max-h-[90vh] relative overflow-y-auto p-8 bg-white rounded-2xl shadow-2xl dark:bg-[#161618] dark:text-white">
          ...
          <div className="pb-4">
            <h2 className="text-xl font-medium">Crear nueva autoridad</h2>
          </div>
          <form
          ...
          >
            ...
            <button className="w-full p-4 text-white font-medium rounded-lg shadow-lg cursor-pointer bg-indigo-600">
              {loading ? "Creando autoridad..." : "Crear autoridad"}
            </button>
          </form>
        </div>
      </dialog>
